...
<?php $i=0; foreach ($data as $d) { $i++; ?>
         <tr class="resultrow<?php if($i % 2) { ?> odd<?php } ?>">
            ...
            <td class="auditcell aaa"><a href="#" onclick="show_modal('removeApproveModal'); Piler.remove_message_id=<?php print $d['id']; ?>;"><i class="bi bi-trash text-danger" title="<?php print $text_remove; ?>"></i></a></td>
            <td class="auditcell aaa"><a href="#" onclick="show_modal('removeRejectModal'); Piler.remove_message_id=<?php print $d['id']; ?>;"><i class="bi bi-ban" title="<?php print $text_cancel; ?>"></i></a></td>
